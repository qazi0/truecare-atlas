...
import { Download, FileJson, FileSpreadsheet, Quote, ShieldCheck, X } from "lucide-react";
...
export default function ShortlistPage() {
  ...
  useEffect(() => {
    void Promise.all(ids.map((id) => fetch(`/api/facility?id=${encodeURIComponent(id)}`).then((r) => r.ok ? r.json() : null).catch(() => null)))
      .then((items) => setFacilities(items.filter(Boolean) as FacilityFull[]));
  }, [ids]);
    ...
    <AppShell>
      <div className="flex flex-1 flex-col gap-5 px-6 py-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex gap-2"><Button size="sm" variant="outline" className="h-9 text-[12px]" onClick={() => setComparison(!comparison)}>Comparison mode</Button><Button size="sm" className="h-9 text-[12px]" onClick={() => exportPlan(ids, format, includeAudit, includeCaps)}><Download className="h-3.5 w-3.5" /> Export</Button></div>
        </div>
            ...
            <table className="w-full text-[12px]">
              ...
              <tbody>
                {facilities.map((facility) => {
                  ...
                  return <tr key={facility.facility_id} className="border-t hairline align-top"><td className="px-3 py-2"><div className="flex items-center gap-2"><TrustRing score={facility.trust_score} size={28} showLabel={false} /><Link href={`/facility/${facility.facility_id}`} className="font-medium hover:underline">{facility.name}</Link></div><div className="ml-9 mt-0.5 text-[11px] text-muted-foreground">{facility.facility_type}</div></td><td className="px-3 py-2">{formatLocation(facility)}</td><td className="px-3 py-2"><div className="flex max-w-[260px] flex-wrap gap-1">{rows.slice(0, 4).map((row) => <CapabilityBadge key={row.key} label={row.label} />)}{rows.length > 4 && <CapabilityBadge label={`+${rows.length - 4}`} dimmed />}</div></td><td className="px-3 py-2 text-right font-mono">{facility.trust_score ?? "-"}</td><td className="px-3 py-2"><StatusBadge status={deriveStatus(facility)} /></td>{comparison && <td className="max-w-[320px] px-3 py-2 text-muted-foreground">{rows[0]?.quote || "No quote captured"}</td>}<td className="px-3 py-2 text-right"><button onClick={() => remove(facility.facility_id)} aria-label={`Remove ${facility.name}`} className="text-muted-foreground hover:text-alert"><X className="h-4 w-4" /></button></td></tr>;
                })}
                {facilities.length === 0 && <tr><td colSpan={comparison ? 7 : 6} className="p-6"><EmptyState title="No facilities selected" detail="Add facilities from Command or Facility Detail." /></td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        <div className="grid gap-5 lg:grid-cols-[1fr_300px]">
          ...
          <div className="rounded-lg border hairline bg-surface p-4">
            ...
            <Button className="mt-4 h-9 w-full text-[12px]" onClick={() => exportPlan(ids, format, includeAudit, includeCaps)}><Download className="h-3.5 w-3.5" /> Export field plan</Button>
          </div>
        </div>
      </div>
    </AppShell>
