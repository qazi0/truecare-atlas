...
SELECT
  ...
  SUM(CASE WHEN trust_score >= 80 AND COALESCE(flag_count, 0) = 0 THEN 1 ELSE 0 END) AS high_trust_count,
  ...
  SUM(CASE WHEN has_icu AND trust_score >= 80 AND COALESCE(flag_count, 0) = 0 THEN 1 ELSE 0 END) AS icu_verified,
  SUM(CAST(has_nicu AS INT)) AS nicu_claimed,
  SUM(CASE WHEN has_nicu AND trust_score >= 80 AND COALESCE(flag_count, 0) = 0 THEN 1 ELSE 0 END) AS nicu_verified,
  SUM(CAST(has_dialysis AS INT)) AS dialysis_claimed,
  SUM(CASE WHEN has_dialysis AND trust_score >= 80 AND COALESCE(flag_count, 0) = 0 THEN 1 ELSE 0 END) AS dialysis_verified,
  SUM(CAST(has_oncology AS INT)) AS oncology_claimed,
  SUM(CASE WHEN has_oncology AND trust_score >= 80 AND COALESCE(flag_count, 0) = 0 THEN 1 ELSE 0 END) AS oncology_verified,
  ...
  SUM(CASE WHEN has_maternity AND trust_score >= 80 AND COALESCE(flag_count, 0) = 0 THEN 1 ELSE 0 END) AS maternity_verified,
  SUM(CAST(has_blood_bank AS INT)) AS blood_bank_claimed,
  SUM(CAST(has_trauma AS INT)) AS trauma_claimed,
  SUM(CASE WHEN has_trauma AND trust_score >= 80 AND COALESCE(flag_count, 0) = 0 THEN 1 ELSE 0 END) AS trauma_verified,
